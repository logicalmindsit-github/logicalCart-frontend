import { Box,  Image, Text,  Spinner,Button,useToast  } from "@chakra-ui/react";
...
const getData = async ({ price, category }) => {
  const res = await axios.get(
    `${site}/products/home?brand=${category}&price=${price}`
  );
  console.log(res.data);
  return res.data;
};

const LaptopHomePro = ({ price, category }) => {
...
console.log("datasdfdf-", data);
  ...
  const handleTheFetch = async () => {
    setLoading(true);
    const append = await getData({ price, category }).then((res) =>
      setData(res)
    );
    setLoading(false);
  };
