...
const LaptopCart = () => {
  ...
  let paid =
    total === 0
      ? 0
      : data.reduce((acc, el) => acc + el.price2 * el.quantity, 0) + 20;
    ...
    <Box pt={2} mb={10}>
      <Flex mb={5} mt="5px" justifyContent={"center"}>
        ...
        <Image
          ...
          w={"200px"}
          h={"50px"}
          src="https://i.postimg.cc/RZhX9rHw/logo.jpg"
        />
      </Flex>
      <Flex justifyContent={"center"}>
        ...
        <Box>
          ...
          <Button
            onClick={() => {
              if (data.length === 0) {
                // If cart is empty, display toast notification
                toast({
                  title: "Your cart is empty",
                  description: "Please add products.",
                  status: "warning",
                  position: "top",
                  duration: 3000, // Optional: Duration for which the toast will be displayed
                  isClosable: true, // Optional: Whether the toast can be closed by user
                });
              } else {
                // If cart is not empty, proceed to checkout
                onOpen(); // Open the checkout modal
              }
            }}
            w="100%"
            bgColor={"purple.500"}
